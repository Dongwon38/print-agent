...
async function printOrder(order) {
  ...
  try {
    log(
      "Sending.ConcurrentModificationException printer initialization command..."
    );
    ...
    if (cart.length === 0) {
      ...
      cart.forEach((item, index) => {
        ...
        const itemNameLength =
          (item.name || item.item_name || "Unknown").length + 3; // "1 x" (3자) 포함
    ...
    log(
      `Print error for order #${order.order_number || "N/A"}: ${error.message}`
    );
  } finally {
